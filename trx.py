...
def reply_to_user(message):
	...
	if message.text == "🉑 TRX":
		with open(f"users/{user_id}/trx_last.txt", 'r') as file:
		    date_str = file.read().strip()
		file_datetime = datetime.strptime(date_str, "%Y-%m-%d %H:%M:%S")
		current_datetime = datetime.now()
		time_difference = current_datetime - file_datetime
		if time_difference > timedelta(hours=1):
			...
			file = open(f"users/{user_id}/trx_balance.txt", "r" , encoding="utf-8")
			old_balance = file.read()
			file.close()
			new_balance = float(old_balance) + 1.0
			file = open(f"users/{user_id}/trx_balance.txt", "w" , encoding="utf-8")
			...
			rkey.row('👛 Кошелек', '🉑 TRX')
			rkey.row('📙 Информация')
			bot.send_message(message.chat.id, f"🐳 Ваш баланс пополнен на 1 TRX", reply_markup=rkey, parse_mode="HTML", disable_web_page_preview = True)
		else:
			...
			rkey.row('👛 Кошелек', '🉑 TRX')
			rkey.row('📙 Информация')
			bot.send_message(message.chat.id, f"⚠️ Сегодня вы уже получили TRX приходи через <b>{formatted_time_remaining}</b> чтобы получить ещё!", reply_markup=rkey, parse_mode="HTML", disable_web_page_preview = True)
	if message.text == "👛 Кошелек":
		...
		bot.send_message(message.chat.id, f"👛 <b>Кошелек</b>\n\n🐳 {message.from_user.username}\n🆔 <code>{message.from_user.id}</code>\n\n🉑 TRX <b>{balance}</b>", reply_markup=inline, parse_mode="HTML", disable_web_page_preview = True)
...
def pressed(call: types.CallbackQuery):
	...
	if command == "balout":
		file = open(f"users/{chatid}/trx_balance.txt", "r" , encoding="utf-8")
		balance = file.read()
		file.close()
		if float(balance) >= 100:	
			...
			bot.send_message(chat_id=chatid, text="⛔️ На вашем балансе, недостаточно <b>TRX\n\nМинимальный вывод 100 TRX</b>", parse_mode="HTML", disable_web_page_preview = True)
	if command == "textmm":
		inline = types.InlineKeyboardMarkup()
...
def balout1(message, message_id):
	...
	if message.text:
		file = open(f"users/{message.chat.id}/trx_balance.txt", "r" , encoding="utf-8")
		balance = file.read()
		file.close()
...
def balout2(message, message_id, summa):
	...
	if message.text:
		...
		file = open(f"users/{message.chat.id}/trx_balance.txt", "w" , encoding="utf-8")
		file.write(f"{new_balance}")
		file.close()
