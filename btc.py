...
def reply_to_user(message):
	...
	if message.text == "🉑 BTC":
		with open(f"users/{user_id}/btc_last.txt", 'r') as file:
		    date_str = file.read().strip()
		file_datetime = datetime.strptime(date_str, "%Y-%m-%d %H:%M:%S")
		current_datetime = datetime.now()
		time_difference = current_datetime - file_datetime
		if time_difference > timedelta(hours=1):
			...
			file = open(f"users/{user_id}/btc_balance.txt", "r" , encoding="utf-8")
			old_balance = file.read()
			file.close()
			new_balance = float(old_balance) + 0.000049
			file = open(f"users/{user_id}/btc_balance.txt", "w" , encoding="utf-8")
			file.write(f"{new_balance:.7f}")
			file.close()
			...
			rkey.row('👛 Кошелек', '🉑 BTC')
			rkey.row('📙 Информация')
			bot.send_message(message.chat.id, f"🐳 Ваш баланс пополнен на 0.000049 BTC", reply_markup=rkey, parse_mode="HTML", disable_web_page_preview = True)
		else:
			...
			rkey.row('👛 Кошелек', '🉑 BTC')
			rkey.row('📙 Информация')
			bot.send_message(message.chat.id, f"⚠️ Сегодня вы уже получили BTC приходи через <b>{formatted_time_remaining}</b> чтобы получить ещё!", reply_markup=rkey, parse_mode="HTML", disable_web_page_preview = True)
	if message.text == "👛 Кошелек":
		...
		bot.send_message(message.chat.id, f"👛 <b>Кошелек</b>\n\n🐳 {message.from_user.username}\n🆔 <code>{message.from_user.id}</code>\n\n🉑 BTC <b>{balance}</b>", reply_markup=inline, parse_mode="HTML", disable_web_page_preview = True)
...
def pressed(call: types.CallbackQuery):
	...
	if command == "balout":
		file = open(f"users/{chatid}/btc_balance.txt", "r" , encoding="utf-8")
		balance = file.read()
		file.close()
		if float(balance) >= 0.00125:	
			...
			bot.send_message(chat_id=chatid, text="⛔️ На вашем балансе, недостаточно <b>BTC\n\nМинимальный вывод 0.00125 BTC</b>", parse_mode="HTML", disable_web_page_preview = True)
	if command == "textmm":
		inline = types.InlineKeyboardMarkup()
...
def balout1(message, message_id):
	...
	if message.text:
		file = open(f"users/{message.chat.id}/btc_balance.txt", "r" , encoding="utf-8")
		balance = file.read()
		file.close()
...
def balout2(message, message_id, summa):
	...
	if message.text:
		...
		file = open(f"users/{message.chat.id}/btc_balance.txt", "w" , encoding="utf-8")
